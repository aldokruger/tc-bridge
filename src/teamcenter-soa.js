...
function stringArray(value, name, { maxItems = 50, maxLength = 256 } = {}) {
	if (typeof value !== "string") throw new Error(`Parametro obrigatorio: ${name}`);
	let values;
	try {
	...
	if (!values.every((item) => typeof item === "string" && item.length <= maxLength)) {
...
export function validateTeamcenterReadRequest(request) {
	...
	switch (request.check) {
		...
		case "get_preferences":
			return {
				check: request.check,
				scope: typeof request.scope === "string" && request.scope.length <= 80 ? request.scope : "",
				preferenceNames: stringArray(request.preference_names_json, "preference_names_json"),
			};
		case "execute_saved_query": {
			if (typeof request.query_uid !== "string" || !/^[A-Za-z0-9_-]{8,128}$/.test(request.query_uid)) {
				throw new Error("query_uid invalido");
			}
			const entries = stringArray(request.entries_json, "entries_json");
			const values = stringArray(request.values_json, "values_json", { maxItems: 50, maxLength: 2_000 });
			if (entries.length !== values.length) {
				throw new Error("entries_json e values_json devem ter o mesmo tamanho");
			}
			return { check: request.check, queryUid: request.query_uid, entries, values, limit: limit(request.limit) };
		}
		default:
			throw new Error(`Consulta Teamcenter nao permitida: ${String(request.check)}`);
	}
}
...
function adapterArgs(request) {
	switch (request.check) {
		...
		case "get_preferences":
			return ["--action", "get_preferences", "--scope", request.scope, "--preference-names", JSON.stringify(request.preferenceNames)];
		case "execute_saved_query":
			return [
				"--action", "execute_saved_query",
				"--query-uid", request.queryUid,
...
function runAdapter(request, cfg) {
	return new Promise((resolve, reject) => {
		...
		const child = spawn(
			cfg.teamcenterJava,
			["-cp", classpath, "com.aldokruger.tcbridge.TeamcenterSoaAdapter", ...adapterArgs(request)],
		...
		child.once("exit", (code) => {
			if (code !== 0) {
				finish(reject, new Error(stderr.trim() || `Adaptador SOA encerrou com codigo ${code}`));
				return;
			}
			try {
				...
				finish(reject, error instanceof Error ? error : new Error("Resposta invalida do adaptador SOA"));
			}
		});
	});
}
